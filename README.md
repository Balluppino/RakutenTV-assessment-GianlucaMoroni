...
```powershell
git clone https://github.com/Balluppino/RakutenTV-assessment-GianlucaMoroni.git
cd RakutenTV-assessment-GianlucaMoroni
```
...
6. Open `.env` in a text editor and configure the API key provider. Once inserted, keys are loaded automatically from `.env`.

7. With the virtual environment active:

```powershell
python backend/app.py
```

8. When it is loaded, open the generated local web page:
